    ...
    if LocalPlayer.state.Character.Group == "admin" then
        ...
        RegisterCommand("horseAtrAll", function(source, args, rawCommand)
            ...
            for index, atr in pairs(ePedAttribute) do
                ...
                print(index - 1, "-", atr, Atributes[atr].Points)
        ...
        RegisterCommand("rechargeStamina",
            function(source, args, rawCommand) -- přidá args[1] k součané stamině koně
        ...
        RegisterCommand("printComp", function(source, args, rawCommand)
            for _, comp in pairs(myHorseComp) do
                print(_, comp)
            end
        end)
        RegisterCommand("unComp", function(source, args, rawCommand)
            -- local playerPed = PlayerPedId()
            -- local comp = args[1] -- componentCategory
            -- local componentHash = tonumber(myHorseComp[comp])

            -- if componentHash == 0 then
            --     print("No component to unequip")
            --     return
            -- end
            -- myHorseComp[comp] = 0
            ...
            -- end
            -- TriggerServerEvent("aprts_horses:equipHorseComponent", myHorse.id, myHorseComp)
            -- UpdatePedVariation(myHorse.ped, 0, 1, 1, 1, false)
            createComponentsMenu()
        end)
    end
end
