...
AddEventHandler('aprts_hunting_job:Server:newWagon', function(wagon, coords, butcherID)
    ...
    if source then
        local firstname = Player(source).state.Character.FirstName
        local lastname = Player(source).state.Character.LastName
        ...
        local message = {
            characterID = Player(source).state.Character.CharId,
            playerName = playerName,
            playerJob = Player(source).state.Character.Job,
            playerGrade = Player(source).state.Character.Grade,
            playerJoblabel = Player(source).state.Character.JobLabel,
			coords = playerCoords,
            butcherID = butcherID,
        }
        lib.logger(source, 'NewJob', message)
    end
end)
...
AddEventHandler('aprts_hunting_job:Server:sellBox', function(butcherID)
    ...
    if source then
        ...
        lib.logger(source, 'SellBox', message)
    end
end)
