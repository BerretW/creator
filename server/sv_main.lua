...
local function DiscordWeb(name, message, footer)
    local embed = {{
...
local function getBaseStats(horseHash)
    return Horses[tostring(horseHash)] and Horses[tostring(horseHash)].stats or nil
end
...
AddEventHandler('aprts_horses:sellHorse', function(identifier)
    print("Selling horse: " .. identifier)
    local _source = source
    local user = Core.getUser(_source)
    if not user then
        notify(_source, 'Uživatel nebyl nalezen.')
    ...
    local price = 0
    local query = tonumber(identifier) and "SELECT * FROM aprts_stables WHERE `id`=@id" or
                      "SELECT * FROM aprts_stables WHERE `vehicles`=@hash LIMIT 1"
    local params = tonumber(identifier) and {
        id = identifier
    } or {
        hash = identifier
    }
        ...
        if result[1] then
            ...
            for _, horse in pairs(Horses) do
                if GetHashKey(horse.horse_id) == tonumber(horseData.vehicles) then
                    price = math.floor(math.max(horse.cashPrice / 10, 30))
                    break
                end
            end
            ...
            DiscordWeb("Prodej koně!", "Hráč " .. GetPlayerName(_source) .. " prodal koně " .. horseData.name ..
                " za " .. price .. " $", "ID: " .. (tonumber(identifier) and horseData.id or horseData.vehicles))
...
AddEventHandler('aprts_horses:newVehicle', function(vehHash, vehType, vehName, metatag, horseBreed)
    ...
    -- Check if the player already owns the horse
    MySQL.query("SELECT * FROM aprts_stables WHERE `identifier`=@identifier AND `charid`=@charid AND `vehicles`=@vehHash AND `type`=@type", {
        identifier = identifier,
        charid = charid,
        vehHash = vehHash,
        type = 'horse'
        ...
        if result[1] then
            notify(_source, "Tento kůň již vlastníš.")
            return
        end
        ...
        local Att = {
            SA_POISONED = { BonusRank = 0, Rank = 0, Points = 0, Name = "SA_POISONED", index = 11 },
            MTR_STRENGTH = { BonusRank = 0, Rank = 0, Points = 0, Name = "MTR_STRENGTH", index = 18 },
            PA_SPECIALABILITY = { BonusRank = 0, Rank = 0, Points = 0, Name = "PA_SPECIALABILITY", index = 2 },
            PA_STAMINA = { BonusRank = 0, Rank = 0, Points = 0, Name = "PA_STAMINA", index = 1 },
            SA_BODYWEIGHT = { BonusRank = 0, Rank = 0, Points = 0, Name = "SA_BODYWEIGHT", index = 13 },
            SA_INEBRIATED = { BonusRank = 0, Rank = 0, Points = 0, Name = "SA_INEBRIATED", index = 10 },
            SA_DIRTINESSSKIN = { BonusRank = 0, Rank = 0, Points = 0, Name = "SA_DIRTINESSSKIN", index = 22 },
            PA_HEALTH = { BonusRank = 0, Rank = 0, Points = 0, Name = "PA_HEALTH", index = 0 },
            PA_COURAGE = { BonusRank = 0, Rank = 0, Points = 0, Name = "PA_COURAGE", index = 3 },
            SA_DIRTINESS = { BonusRank = 0, Rank = 0, Points = 0, Name = "SA_DIRTINESS", index = 16 },
            PA_AGILITY = { BonusRank = 0, Rank = 0, Points = 0, Name = "PA_AGILITY", index = 4 },
            SA_OVERFED = { BonusRank = 0, Rank = 0, Points = 0, Name = "SA_OVERFED", index = 14 },
            SA_SICKNESS = { BonusRank = 0, Rank = 0, Points = 0, Name = "SA_SICKNESS", index = 15 },
            SA_BODYHEAT = { BonusRank = 0, Rank = 0, Points = 0, Name = "SA_BODYHEAT", index = 12 },
            PA_SPEED = { BonusRank = 0, Rank = 0, Points = 0, Name = "PA_SPEED", index = 5 },
            SA_DIRTINESSHAT = { BonusRank = 0, Rank = 0, Points = 0, Name = "SA_DIRTINESSHAT", index = 17 },
            MTR_INSTINCT = { BonusRank = 0, Rank = 0, Points = 0, Name = "MTR_INSTINCT", index = 20 },
            SA_HUNGER = { BonusRank = 0, Rank = 0, Points = 0, Name = "SA_HUNGER", index = 8 },
            PA_ACCELERATION = { BonusRank = 0, Rank = 0, Points = 0, Name = "PA_ACCELERATION", index = 6 },
            PA_BONDING = { BonusRank = 0, Rank = 0, Points = 0, Name = "PA_BONDING", index = 7 },
            PA_UNRULINESS = { BonusRank = 0, Rank = 0, Points = 0, Name = "PA_UNRULINESS", index = 21 },
            SA_FATIGUED = { BonusRank = 0, Rank = 0, Points = 0, Name = "SA_FATIGUED", index = 9 },
            MTR_GRIT = { BonusRank = 0, Rank = 0, Points = 0, Name = "MTR_GRIT", index = 19 }
        }
        ...
        MySQL.execute(
            ...
            function(rowsChanged)
                if rowsChanged > 0 then
                    notify(_source, "Kůň byl úspěšně přidán do stáje.")
                    DiscordWeb("Nový Kůň", "Hráč " .. GetPlayerName(_source) .. " přidal nového koně " .. vehName .. ".", "ID: " .. vehHash)
                else
                    notify(_source, "Něco se nepovedlo při přidávání koně do stáje.")
                end
            end
        )
    end)
...
-- Flee Horse
RegisterServerEvent("aprts_horses:Server:fleeHorse")
AddEventHandler("aprts_horses:Server:fleeHorse", function(horseID)
    TriggerClientEvent("aprts_horses:Client:fleeHorse", -1, horseID)
end)
