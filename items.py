...
class ItemDialog(QtWidgets.QDialog):
    ...
    def load_item_image(self):
        ...
        # Pokud není zadán image_file, použijeme 'default.png'
        if not image_file:
        ...
        if os.path.exists(cache_file_path):
            ...
            try:
                ...
                # Pokud se nepodaří stáhnout obrázek, použijeme default.png
                default_image_path = os.path.join(self.cache_dir, 'default.png')
                if not os.path.exists(default_image_path):
                    # Stáhneme default.png z URL
                    default_url = "https://api.westhavenrp.cz/storage/items/default.png"
                    try:
                        req = Request(default_url, headers={'User-Agent': 'Mozilla/5.0'})
                        data = urlopen(req).read()
...
if __name__ == '__main__':
    ...
    if not os.path.exists(config_path):
        QtWidgets.QMessageBox.critical(None, "Chyba", f"Soubor konfigurace {config_path} nebyl nalezen.")
        sys.exit(1)
    with open(config_path, 'r') as f:
        config = json.load(f)
    # Vytvoříme připojení k databázi
    try:
        ...
        QtWidgets.QMessageBox.critical(None, "Chyba", f"Nelze se připojit k databázi: {err}")
        sys.exit(1)
