...
function StartPhonograph(handle, url, volume, offset, filter, locked, video, videoSize, muted)
    ...
    print('Starting phonograph', handle, url, volume, offset, filter, locked, video, videoSize, muted)
...
function CreatePhonograph(phonograph)
    ...
    phonograph.handle = CreateObjectNoOffset(GetHashKey('p_phonograph01x'), phonograph.x, phonograph.y, phonograph.z,
...
function IsPauseMenuOrMapActive()
    return IsPauseMenuActive() or IsAppActive(GetHashKey('MAP')) ~= 0
...
AddEventHandler('phonograph:Client:play', function(preset)
    StartClosestPhonograph(preset, 50, nil, false, false, false, 50, false)
end)
...
RegisterNUICallback('init', function(data, cb)
    if NetworkDoesNetworkIdExist(data.handle) or data.coords then
        ...
        TriggerServerEvent('phonograph:init', data.handle, data.url, data.title, data.volume, data.offset, data.filter,
            data.locked, data.video, data.videoSize, data.muted, coords and tovector3(coords))
    end
    cb({})
end)
...
RegisterNUICallback('play', function(data, cb)
    StartPhonograph(data.handle, data.url, data.volume, data.offset, data.filter, data.locked, data.video,
        data.videoSize, data.muted)
    cb({})
end)
...
--     TriggerEvent('chat:addSuggestion', '/phono',
--         'Interact with phonographs. No arguments will open the phonograph control panel.', {{
--             name = 'command',
--             help = 'play|pause|stop|status|songs'
--         }, {
--             name = 'url',
--             help = 'URL or preset name of music to play. Use "random" to play a random preset.'
--         }, {
--             name = 'volume',
--             help = 'Volume to play the music at (0-100).'
--         }, {
--             name = 'time',
--             help = 'Time in seconds to start playing at.'
--         }, {
--             name = 'filter',
--             help = '0 = normal audio, 1 = add phonograph filter'
--         }, {
--             name = 'lock',
--             help = '0 = unlocked, 1 = locked'
--         }, {
--             name = 'video',
--             help = '0 = hide video, 1 = show video'
--         }, {
--             name = 'size',
--             help = 'Video size'
--         }, {
        ...
        for handle, info in pairs(Phonographs) do
            ...
            if object and object > 0 then
                ...
                local onScreen, screenX, screenY =
                    GetScreenCoordFromWorldCoord(phonoPos.x, phonoPos.y, phonoPos.z + 0.8)
...
CreateThread(function()
    while true do
        ...
        if phono then
            ...
            DrawText3D(coords.x, coords.y, coords.z + 1.0, "[~s~G~s~] STOP")
