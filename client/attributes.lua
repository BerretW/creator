...
function displayHorseAttributes(horse, showImage, imagePath)
    ...
    local data = {{
        text = "Zdraví: " .. coreAttributes.CoreHealth .. "/" .. attributes.PA_HEALTH.Points,
        line = false,
        value = coreAttributes.Health,
        color = "#ff0000",
        lineColor = "#ff0000"
    }, {
        text = "Stamina: " .. coreAttributes.CoreStamina .. "/" .. attributes.PA_STAMINA.Points,
        line = false,
...
AddEventHandler("aprts_horses:useItem", function(item)
    ...
    if isMounted == true then
        ...
        if myhorse == true then
            TriggerEvent("aprts_horses:updateAttributes", myHorse.id, myHorse.ped)
        end
    else
...
AddEventHandler("aprts_horses:updateAttributes", function(horseID, horse, shoed)
    ...
    TriggerServerEvent("aprts_horses:Server:updateAttributes", horseID, Attributes, CoreAttributes)
end)
...
AddEventHandler("aprts_horses:restoreStamina", function(horse, points, horseID)
    ...
    if horseID then
        TriggerEvent("aprts_horses:updateAttributes", horseID, horse)
    end
